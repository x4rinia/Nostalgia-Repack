...
BINDING_NAME_DMM_SLOT_1 = "Spell chain 1 trigger";
BINDING_NAME_DMM_SLOT_2 = "Spell chain 2 trigger";
BINDING_NAME_DMM_SLOT_3 = "Spell chain 3 trigger";
BINDING_NAME_DMM_SLOT_4 = "Spell chain 4 trigger";
BINDING_NAME_DMM_SLOT_5 = "Spell chain 5 trigger";
BINDING_NAME_DMM_SLOT_6 = "Spell chain 6 trigger";
BINDING_NAME_DMM_SLOT_7 = "Spell chain 7 trigger";
BINDING_NAME_DMM_SLOT_8 = "Spell chain 8 trigger";
BINDING_NAME_DMM_SLOT_9 = "Spell chain 9 trigger";
BINDING_NAME_DMM_SLOT_10 = "Spell chain 10 trigger";
BINDING_NAME_DMM_SLOT_11 = "Spell chain 11 trigger";
BINDING_NAME_DMM_SLOT_12 = "Spell chain 12 trigger";
BINDING_NAME_DMM_SLOT_13 = "Spell chain 13 trigger";
BINDING_NAME_DMM_SLOT_14 = "Spell chain 14 trigger";
...
local DMM = {
    ...
    PET_ATTACK_ICON = "Interface\\Icons\\Ability_GhoulFrenzy",
    BOT_ATTACK_ICON = "Interface\\Icons\\Ability_Warrior_OffensiveStance",
...
function DMM.IsPetAttack(entry)
    return type(entry) == "table" and entry.kind == "command" and entry.command == "PETATTACK";
end

function DMM.NewPetAttackEntry()
    return {
        kind = "command",
        command = "PETATTACK",
        name = "Pet: Attack",
        rank = "",
        icon = DMM.PET_ATTACK_ICON
    };
end

function DMM.IsBotAttack(entry)
    return type(entry) == "table" and entry.kind == "command" and entry.command == "BOTATTACK";
end

function DMM.IsCommand(entry)
    return DMM.IsPetAttack(entry) or DMM.IsBotAttack(entry);
end

function DMM.NewBotAttackEntry()
    return {
        kind = "command",
        command = "BOTATTACK",
...
function DMM.SpellMatches(bookIndex, bookType, spell)
    ...
    rank = rank or "";
    local targetRank = spell.rank or "";
    if targetRank ~= "" and rank ~= targetRank then
        return nil;
    end
    return 1;
end
...
function DMM.ResolveSpell(spell)
    ...
    if DMM.IsPetAttack(spell) then
        spell.icon = DMM.PET_ATTACK_ICON;
        return type(PetAttack) == "function" and spell.command or nil;
    end
    if DMM.IsBotAttack(spell) then
        spell.icon = DMM.BOT_ATTACK_ICON;
...
function DMM.IsSpellEntryUsable(spell)
    if type(spell) ~= "table" or type(spell.name) ~= "string" then
        return false, false;
    end
    ...
    if type(IsUsableSpell) == "function" then
        ...
        if ok and isUsable ~= nil and not isUsable then
            return false, notEnoughMana;
        end
    end
...
function DMM.IsSpellOnCooldownOrUnavailable(spell)
    if type(spell) ~= "table" or type(spell.name) ~= "string" then
        return true;
    end
...
function DMM.GetEffectiveStep(slot)
    if not slot or type(slot.spells) ~= "table" then
        return 1;
    end
    ...
    local current = slot.current or 1;
    if current < 1 or current > count then
        current = 1;
    end

    for offset = 0, count - 1 do
        local stepIndex = math.mod(current - 1 + offset, count) + 1;
        local spell = slot.spells[stepIndex];
        if spell and not DMM.IsSpellOnCooldownOrUnavailable(spell) then
            return stepIndex;
        end
    end
...
function DMM.UpdateSlotButton(index)
    ...
    local count = table.getn(slot.spells);
    local cur = DMM.GetEffectiveStep(slot);
    ...
    if not bookIndex then
        ...
        button.step:SetText(cur .. "/" .. count);
        button:SetBackdropBorderColor(0.25, 0.65, 0.95, 1);
        slot.invalid = nil;
        if button.cooldown then
            local start, duration, enable = 0, 0, 0;
            if not DMM.IsCommand(spell) then
                start, duration, enable = GetSpellCooldown(bookIndex, spell.bookType or DMM.BOOKTYPE);
            end
...
function DMM.UpdateEditorFields()
    for i = 1, DMM.MAX_SPELLS do
        ...
        if spell then
            ...
            if DMM.IsPetAttack(spell) then
                field.hint:SetText("Pet command | Right-click: remove");
            elseif DMM.IsBotAttack(spell) then
                field.hint:SetText("Bot command | Right-click: remove");
            else
                field.hint:SetText("Spell | Right-click: remove");
            end
            ...
            field.text:SetText("Step " .. i .. ": drop spell here");
            field.hint:SetText("Left-click: Pet | Shift-click: Bot");
            field:SetBackdropBorderColor(0.3, 0.4, 0.5, 1);
        end
    end
end

function DMM.InsertPetAttack(index)
    if DMM.editorSpells[index] then
        DMM.SetEditorStatus("Clear the field with a right-click first.", 1);
        return;
    end
    DMM.editorSpells[index] = DMM.NewPetAttackEntry();
    DMM.UpdateEditorFields();
    DMM.SetEditorStatus("Pet: Attack added (not saved yet).", nil);
end

function DMM.InsertBotAttack(index)
    if DMM.editorSpells[index] then
        DMM.SetEditorStatus("Clear the field with a right-click first.", 1);
        return;
    end
...
function DMM.RemoveEditorSpell(index)
    ...
    DMM.UpdateEditorFields();
    DMM.SetEditorStatus("Spell step removed.", nil);
end
...
function DMM.ReceiveSpell(index)
    ...
    if type(GetCursorInfo) == "function" then
        ...
        if cursorType ~= "spell" then
            DMM.SetEditorStatus("Drop a player spell from the spellbook.", 1);
            return;
        end
    else
        ...
        if not bookIndex then
            DMM.SetEditorStatus("The spellbook reference could not be read.", 1);
            return;
        end
    end
    bookType = bookType or DMM.BOOKTYPE;
    if bookType ~= DMM.BOOKTYPE then
        DMM.SetEditorStatus("Pet spells are not supported.", 1);
        return;
    end
    local name, rank = GetSpellName(bookIndex, bookType);
    if not name then
        DMM.SetEditorStatus("The dropped spell could not be read.", 1);
        return;
    end
    if IsSpellPassive and IsSpellPassive(bookIndex, bookType) then
        DMM.SetEditorStatus("Passive spells cannot be triggered.", 1);
        return;
    end
    ...
    DMM.UpdateEditorFields();
    DMM.SetEditorStatus(name .. " saved locally (not applied yet).", nil);
end
...
function DMM.OpenEditor(index)
    ...
    DMM.editorFrame.title:SetText("Slot " .. index .. " edit");
    ...
    DMM.SetEditorStatus("Choose 2 to 4 spells or pet commands.", nil);
    DMM.UpdateEditorFields();
...
function DMM.SaveEditor()
    ...
    if count < DMM.MIN_SPELLS or count > DMM.MAX_SPELLS then
        DMM.SetEditorStatus("A chain requires 2 to 4 steps.", 1);
        return;
    end
    for i = 1, count do
        if not DMM.ResolveSpell(compact[i]) then
            DMM.SetEditorStatus("Step " .. i .. " is no longer available.", 1);
            return;
        end
    end
    ...
    if not resetSeconds or resetSeconds < 0 or resetSeconds > 3600 then
        DMM.SetEditorStatus("Reset time must be between 0 and 3600 seconds.", 1);
        return;
    end
...
function DMM.ClearEditorSlot()
    ...
    DMM.UpdateEditorFields();
    DMM.UpdateAllSlots();
    DMM.SetEditorStatus("The entire slot was cleared.", nil);
    DMM.SetStatus("Slot " .. DMM.editorSlot .. " cleared.", nil);
end
...
function DMM.EditorHasUnsavedChanges()
    ...
    for i = 1, editorCount do
        ...
        if eSpell.name ~= sSpell.name or (eSpell.rank or "") ~= (sSpell.rank or "") or
           eSpell.kind ~= sSpell.kind or eSpell.command ~= sSpell.command then
            return true;
        end
    end
...
function DinoMacroManager_GetSlotCooldown(slotIndex)
    ...
    if DMM.IsCommand(spell) then return 0, 0, 0; end
    local start, duration, enable = GetSpellCooldown(bookIndex, spell.bookType or DMM.BOOKTYPE);
...
function DMM.PickupSlotMacro()
    ...
    if DMM.EditorHasUnsavedChanges() then
        DMM.SetEditorStatus("Save the chain first!", 1);
        if UIErrorsFrame and UIErrorsFrame.AddMessage then
            UIErrorsFrame:AddMessage("Save the chain first!", 1.0, 0.2, 0.2, 1.0);
        else
            DMM.Chat("Save the chain before dragging it to an action bar.");
        end
        return;
    end
    ...
    if count < DMM.MIN_SPELLS then
        DMM.SetEditorStatus("A chain requires at least " .. DMM.MIN_SPELLS .. " spells.", 1);
        return;
    end

    if type(CreateMacro) ~= "function" or type(PickupMacro) ~= "function" then
        DMM.SetEditorStatus("This client does not provide the macro APIs.", 1);
        return;
    end
    ...
    if not macroIndex or macroIndex == 0 then
        DMM.SetEditorStatus("No free character-specific macro slot.", 1);
        return;
    end
    ClearCursor();
    PickupMacro(macroIndex);
    DMM.SetEditorStatus("DMM Slot " .. index .. " is on the cursor: drag it onto an action bar.", nil);
end
...
function DMM.ConfirmPendingCast()
    ...
    DMM.UpdateBlizzardCooldowns();
    DMM.SetStatus("Slot " .. pending.slotIndex .. ": next step " .. slot.current .. ".", nil);
end
...
function DinoMacroManager_TriggerSlot(index)
    ...
    local count = table.getn(slot.spells);
    if count < DMM.MIN_SPELLS or count > DMM.MAX_SPELLS then
        DMM.SetStatus("Slot " .. index .. " is empty or incomplete.", 1);
        DMM.OpenEditor(index);
        return;
    end
    if DMM.pendingCast then
        DMM.SetStatus("A chain step is still waiting for confirmation.", 1);
        return;
    end

    local stepToUse = DMM.GetEffectiveStep(slot);
    slot.current = stepToUse;

    local spell = slot.spells[stepToUse];
    local bookIndex = DMM.ResolveSpell(spell);
    if not bookIndex then
        slot.invalid = 1;
        DMM.UpdateSlotButton(index);
        DMM.SetStatus("Slot " .. index .. ": current spell is invalid.", 1);
        return;
    end

    local start, duration, enabled = 0, 0, 1;
    if not DMM.IsCommand(spell) then
        start, duration, enabled = GetSpellCooldown(bookIndex, spell.bookType or DMM.BOOKTYPE);
    end
    if enabled == 0 or (start and duration and duration > 0 and start + duration > GetTime()) then
        DMM.SetStatus("Slot " .. index .. ": current step is not ready.", 1);
        return;
    end
    ...
    if DMM.IsPetAttack(spell) then
        if not UnitExists("pet") or UnitIsDead("pet") then
            DMM.pendingCast = nil;
            DMM.SetStatus("Slot " .. index .. ": no active pet.", 1);
            return;
        end
        if not UnitExists("target") or UnitIsDead("target") or not UnitCanAttack("player", "target") then
            DMM.pendingCast = nil;
            DMM.SetStatus("Slot " .. index .. ": no attackable target.", 1);
            return;
        end
    ...
    if DMM.IsBotAttack(spell) then
        ...
        DMM.pendingCast.successAt = GetTime() + DMM.SUCCESS_DELAY;
        if DinoController_BotAttack then
            DinoController_BotAttack();
        else
    ...
    if groundResult == "no_target" then
        DMM.pendingCast = nil;
        DMM.SetStatus("Slot " .. index .. ": no valid target selected for the ground spell.", 1);
    elseif groundResult == "unresolved" then
        DMM.pendingCast = nil;
        DMM.SetStatus("Slot " .. index .. ": ground spell could not be resolved.", 1);
    end
end
...
function DMM.ResetPosition()
    ...
    DMM.RestorePosition();
    DMM.SetStatus("Window position reset.", nil);
end
...
function DMM.CreateSlotButton(index)
    ...
    button:SetScript("OnEnter", function()
        ...
        if spell then
            GameTooltip:AddLine("Next step: " .. spell.name, 1, 1, 1);
        end
        GameTooltip:AddLine("Left-click: trigger", 0.6, 0.85, 1);
        GameTooltip:AddLine("Right-click: edit", 0.6, 0.85, 1);
        GameTooltip:Show();
    end);
...
function DMM.CreateEditor()
    ...
    for i = 1, DMM.MAX_SPELLS do
        ...
        field:SetScript("OnClick", function()
            ...
            elseif arg1 == "LeftButton" and not DMM.editorSpells[fieldIndex] then
                if IsShiftKeyDown() then
                    DMM.InsertBotAttack(fieldIndex);
                else
                    DMM.InsertPetAttack(fieldIndex);
                end
            end
        end);
    ...
    resetLabel:SetText("Time reset after:");
    ...
    resetSecondsLabel:SetText("seconds (0 = off)");
    ...
    targetResetLabel:SetText("Reset to spell 1 when target changes");
    ...
    actionBar:SetText("Drag to action bar");
...
function DMM.OnUpdate(elapsed)
    ...
    if DMM.pendingCast then
        ...
        elseif now - DMM.pendingCast.startedAt > DMM.PENDING_TIMEOUT then
            DMM.pendingCast = nil;
        DMM.SetStatus("Cast confirmation timed out; step unchanged.", 1);
        end
    end
    ...
    if changed then
        DMM.SetStatus("Inactive chain reset to step 1.", nil);
    end
    DMM.UpdateBlizzardCooldowns();
end
...
function DMM.OnEvent(eventName, firstArg)
    if eventName == "ADDON_LOADED" then
        if firstArg == "DinoMacroManager" then
            DMM.InitializeDB();
    ...
    if eventName == "PLAYER_TARGET_CHANGED" then
        ...
        if changed then
            DMM.UpdateBlizzardCooldowns();
            DMM.SetStatus("Target changed: marked chains reset to step 1.", nil);
        end
        return;
    end
    ...
    elseif eventName == "SPELLCAST_FAILED" or eventName == "SPELLCAST_INTERRUPTED" or eventName == "UI_ERROR_MESSAGE" then
        DMM.pendingCast = nil;
        DMM.SetStatus("Spell failed; step unchanged.", 1);
    end
end
...
dmmEventFrame:RegisterEvent("ADDON_LOADED");
dmmEventFrame:RegisterEvent("VARIABLES_LOADED");
dmmEventFrame:RegisterEvent("PLAYER_LOGIN");
dmmEventFrame:RegisterEvent("PLAYER_ENTERING_WORLD");
dmmEventFrame:RegisterEvent("PLAYER_LOGOUT");
