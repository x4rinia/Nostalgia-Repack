...
CreateButton("AOE", "Bots use their area abilities.  (.x aoe)", ".x aoe", 68, 228, -74, 0.34, 0.18, 0.55)
...
local function ToggleFollow()
    if followToggle then
        ...
        followButton.tooltip = "Bots follow you.  (.x follow)"
    else
        SendCommand(".x follow")
        followButton:SetText("SUMMON")
        followButton.tooltip = "Summons bots to you.  (.x summon)"
    end
    followToggle = not followToggle
end
followButton = CreateButton("FOLLOW", "Bots follow you.  (.x follow)", ToggleFollow, 68, 12, -104, 0.15, 0.37, 0.52)
...
local function TogglePause()
    if pauseToggle then
        ...
        pauseButton.tooltip = "Pauses the bots.  (.x pause)"
    else
        SendCommand(".x pause")
        pauseButton:SetText("RESUME")
        pauseButton.tooltip = "Resumes the bots.  (.x unpause)"
    end
    pauseToggle = not pauseToggle
end
pauseButton = CreateButton("PAUSE", "Pauses the bots.  (.x pause)", TogglePause, 68, 84, -104, 0.34, 0.34, 0.16)

CreateButton("REVIVE", "Bots revive fallen party members.  (.x rezz)", ".x rezz", 68, 156, -104, 0.23, 0.42, 0.26)
...
local function MarkCrowdControl()
    if not UnitExists("target") then
        DEFAULT_CHAT_FRAME:AddMessage("|cffff4040XBot: No target selected.|r")
        return
    end
    ...
    ccButton.tooltip = "Marks the target for crowd control. Next marker: " .. ccMarks[ccIndex].label .. "."
end
ccButton = CreateButton("CC: Star", "Marks the target for crowd control. Next marker: Star.", MarkCrowdControl, 68, 228, -104, 0.31, 0.18, 0.50)
...
CreateButton("USE", "Bots use their target or an item.  (.x use)", UseButton, 68, 12, -134, 0.27, 0.30, 0.36)
CreateButton("HONOR", "Runs the dishonorable-kill command.  (.x morde)", ".x morde", 68, 84, -134, 0.42, 0.16, 0.22)
CreateButton("XP", "Runs the XP command.  (.x xp)", ".x xp", 68, 156, -134, 0.17, 0.39, 0.34)
CreateButton("/dino", "Open controller settings", function()
    if SlashCmdList and SlashCmdList["DINOCONTROLLER"] then
        SlashCmdList["DINOCONTROLLER"]("")
    elseif ChatFrameEditBox then
        ChatFrameEditBox:SetText("/dino")
        ChatEdit_SendText(ChatFrameEditBox)
    end
...
footer:SetText("/xbot  |  Drag to move")
...
minimapButton:SetScript("OnEnter", function()
    ...
    GameTooltip:AddLine("Left-click: open PartyBot controls", 0.8, 0.8, 0.8, true)
    GameTooltip:AddLine("Drag: move position", 0.8, 0.8, 0.8, true)
    GameTooltip:Show()
end)
