...
local RefreshElements
local UpdateHighlight
...
local function PerformScroll(element, direction)
    ...
    if element.scrollAction then
        ...
        if not ok then
            UIPrint("Scroll error: " .. tostring(res))
            return false
        end
        return res ~= false
    end
...
local function ResolveMerchantSlot(button)
    ...
    if not slot or slot <= 0 then
        ...
        local _, _, itemIndex = string.find(name, "^MerchantItem(%d+)ItemButton$")
        if itemIndex then
...
local function ActivateMerchantButton(button)
    ...
    if selectedTab == 1 then
        ...
        if not ok then
            UIPrint("Purchase failed: " .. tostring(err))
            return nil
        end
        return 1
    else
        ...
        if not ok then
            UIPrint("Buyback failed: " .. tostring(err))
            return nil
        end
        return 1
    end
...
local function ActivateContainerItemInMerchant(button)
    ...
    if bagID and slotID then
        ...
        if not ok then
            UIPrint("Sale failed: " .. tostring(err))
            return nil
        end
        return 1
    end
...
local function ActivateMerchantRepairItem(button)
    if not button or not button:IsVisible() then return nil end
    ...
    if InRepairMode and InRepairMode() then
        if not CharacterFrame or not CharacterFrame:IsVisible() then
            ...
            elseif ShowUIPanel and CharacterFrame then
        ...
        if state.elements then
            ...
            for i = 1, table.getn(state.elements) do
                ...
                if elem and (elem.key == "CharacterHeadSlot" or (elem.key and string.find(elem.key, "^Character.+Slot$"))) then
                    state.index = i
                    UpdateHighlight()
                    break
                end
...
local function SellAllGreyItems()
    if not MerchantFrame or not MerchantFrame:IsVisible() then return nil end
    ...
    if totalSold > 0 then
        UIPrint(totalSold .. " gray items sold.")
    else
        UIPrint("No gray items found to sell.")
    end
    return 1
end

local function EnsureMerchantSellJunkButton()
...
local function AddMerchant(elements)
    local junkBtn = EnsureMerchantSellJunkButton()
    if UsableButton(junkBtn) then
        AddFrameButton(elements, "MerchantSellJunkButton", junkBtn, "LeftButton", function()
            return SellAllGreyItems()
        end)
    end
    ...
    local repairItemBtn = getglobal("MerchantRepairItemButton")
    if UsableButton(repairItemBtn) then
        AddButton(elements, "MerchantRepairItemButton", "LeftButton", function()
            return ActivateMerchantRepairItem(repairItemBtn)
        end)
    else
    ...
    if Visible("CharacterFrame") then
        local idx
        for idx = 1, table.getn(characterSlots) do
            local slotBtn = getglobal(characterSlots[idx])
            if UsableButton(slotBtn) then
                AddButton(elements, characterSlots[idx], "LeftButton")
            end
        end
    end
...
local function ActivateLootButton(button)
    ...
    if not ok then
        UIPrint("Loot slot " .. tostring(slot) .. " could not be looted: " .. tostring(message))
        return nil
    end
    return 1
end
...
local GROUP_LOOT_ACTIONS = {
    { suffix = "IconFrame", choice = "icon", rollType = nil },
...
local function ActivateGroupLootRoll(rollFrame, rollType)
    ...
    if not ok then
        UIPrint("Loot roll failed: " .. tostring(err))
        return nil
    end
    return 1
end
...
local function AddGroupLootRollAction(elements, rollFrame, frameIndex, actionInfo)
    ...
    AddFrameButton(elements, buttonName, button, "LeftButton", function()
        if button and button.Click then
            pcall(function() button:Click("LeftButton") end)
        end
        if actionInfo.rollType then
...
local function AddGroupLootRolls(elements)
    ...
    if table.getn(elements) > originalCount then
        if Visible("LootFrame") and CloseLoot then
            CloseLoot()
        end
        return true
    end
...
local function FindInitialBankSlot(elements)
    local emptyIndex = nil
    local index
    for index = 1, table.getn(elements) do
        local elem = elements[index]
        local key = elem.key or ""
        if string.find(key, "^BankFrameItem%d+$") then
            local slotID = elem.frame and elem.frame:GetID()
            if slotID and slotID > 0 then
                if GetContainerItemInfo and GetContainerItemInfo(-1, slotID) then
                    return index
                elseif not emptyIndex then
                    emptyIndex = index
                end
            end
        end
    end
    if emptyIndex then return emptyIndex end
    return FindFirstMatchingElement(elements, "^BankFrameItem1$")
end
...
local function AddBank(elements)
    ...
    local bagIdx
    for bagIdx = 1, 6 do
        local button = getglobal("BankFrameBag" .. bagIdx)
        if button then
            AddFrameButton(elements, "BankFrameBag" .. bagIdx, button, "LeftButton", nil, true)
        end
    end
...
local function AddCharacter(elements)
    AddContainers(elements, "RightButton")
    local index
    if Visible("PaperDollFrame") then
    ...
    if Visible("ReputationFrame") then
        ...
        AddRange(elements, "ReputationBar", 1, 15)
    ...
    elseif Visible("SkillFrame") then
        AddButton(elements, "SkillFrameCollapseAllButton")
    ...
    elseif Visible("HonorFrame") then
        -- HonorFrame support
    end
    AddRange(elements, "CharacterFrameTab", 1, 5)
...
local function LeaveUIMode()
    ...
    state.itemHoldStart = nil
    state.itemHoldTriggered = nil
    if highlight:GetParent() ~= UIParent then
        highlight:SetParent(UIParent)
        highlight:SetFrameStrata("TOOLTIP")
    end
...
RefreshElements = function()
    ...
    if context == "lootroll" and oldContext ~= "lootroll" then
        preferredIndex = FindFirstMatchingElement(elements, "^GroupLootFrame%d+IconFrame$") or
                         FindFirstMatchingElement(elements, "^GroupLootFrame%d+RollButton$") or
                         FindFirstMatchingElement(elements, "^GroupLootFrame%d+GreedButton$") or
                         FindFirstMatchingElement(elements, "^GroupLootFrame%d+PassButton$") or 1
    ...
    elseif context == "bank" and (state.pendingBankFocus or context ~= oldContext) then
        preferredIndex = FindInitialBankSlot(elements)
...
function DinoController_UIMoveDir(dir)
    ...
    if curElem then
        ...
        elseif curElem.key and (dir == "UP" or dir == "DOWN") then
            if string.find(curElem.key, "^SkillRankFrame") then
                local scrollTarget = "SkillListScrollFrameScrollBar"
                if (dir == "DOWN" and curElem.key == "SkillRankFrame12") or
                   (dir == "UP" and curElem.key == "SkillRankFrame1") then
                    local scrolled = PerformScroll({ isScrollArea = true, scrollTarget = scrollTarget }, dir)
                    if scrolled then
                        local oldKey = curElem.key
                        RefreshElements()
                        local idx
                        for idx = 1, table.getn(state.elements) do
                            if state.elements[idx].key == oldKey then
                                state.index = idx
                                break
                            end
                        end
            ...
            elseif string.find(curElem.key, "^ReputationHeader") or string.find(curElem.key, "^ReputationBar") then
                local scrollTarget = "ReputationListScrollFrameScrollBar"
                if (dir == "DOWN" and (curElem.key == "ReputationHeader15" or curElem.key == "ReputationBar15")) or
                   (dir == "UP" and (curElem.key == "ReputationHeader1" or curElem.key == "ReputationBar1")) then
                    local scrolled = PerformScroll({ isScrollArea = true, scrollTarget = scrollTarget }, dir)
                    if scrolled then
                        local oldKey = curElem.key
                        RefreshElements()
...
function DinoController_UIActivate()
    ...
    local activeElement = state.elements and state.elements[state.index]
    RefreshElements()
    if not state.active then return end
    local element = state.elements[state.index]
    if not element then element = activeElement end
    ...
    if not ok then
        UIPrint("This UI element could not be activated: " .. tostring(err))
    end
    if state.context == "talent" or state.context == "lootroll" then RefreshElements() end
    state.nextScan = 0
end
...
navigationFrame:SetScript("OnEvent", function()
    ...
    elseif event == "BANKFRAME_OPENED" then
        ...
        OpenAllBags(1)
        local numBankSlots = (GetNumBankSlots and GetNumBankSlots()) or 0
        local bagIdx
        for bagIdx = 1, numBankSlots do
            if OpenBag then OpenBag(bagIdx + 4) end
        end
